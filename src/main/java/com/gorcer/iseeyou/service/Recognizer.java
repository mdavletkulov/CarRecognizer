...
import com.gorcer.iseeyou.config.RecognizeConfig;
            ...
            if (contours.elem_size() > 0) {
                ...
                if ((approx.total() == 4 // Четыре стороны                    
                    ...
                    if (maxCosine < config.maxCosine) {
                        ...
                        if (rect.width() > rect.height() &&  // Ширина больше высоты
                                (rect.width() / (float) img.width() < 0.9) && (rect.height() / (float) img.height() < 0.9)) {
                            squares.add(approx);
                        }
